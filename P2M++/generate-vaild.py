...
from modules.models_mvp2m import MeshNet
...
def main(cfg):
    ...
    model_dir = os.path.join(cfg.save_path, cfg.name, 'models_vaild')
    predict_dir = os.path.join(cfg.save_path, cfg.name, 'coarse_intermediate_vaild', str(step))
    if not os.path.exists(predict_dir):
        os.makedirs(predict_dir)
        print('==> make vaild predict_dir {}'.format(predict_dir))
    # -------------------------------------------------------------------
    print('=> build model')
    # Define model
    ...
    # ---------------------------------------------------------------
    print('=> load data')
    data = DataFetcher(file_list=cfg.v_coarse_result_file_path,
                       data_root=cfg.v_coarse_result_data_path,
                       image_root=cfg.v_coarse_result_image_path, is_val=False)
    data.setDaemon(True)
    data.start()
    # ---------------------------------------------------------------
    ...
    pkl = pickle.load(open('data/iccv_p2mpp.dat', 'rb')) #椭圆体
   # Construct Feed dict
    ...
    for iters in range(test_number):
        ...
        print('Vaild Iteration {}/{}, Data id {}'.format(iters + 1, test_number, data_id))
