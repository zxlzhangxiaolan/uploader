...
class LocalGraphProjection(Layer):
    ...
    def _call(self, inputs):
        ...
        for i in range(self.view_number):
            ...
            y = w / (224.0 / 224)
            flag = 0
            out1 = self.bi_linear_sample(self.img_feat[0], n, x, y, flag)
            out1_list.append(out1)
            x = h / (224.0 / 112)
            y = w / (224.0 / 112)
            flag = 1
            out2 = self.bi_linear_sample(self.img_feat[1], n, x, y, flag)
            out2_list.append(out2)
            x = h / (224.0 / 56)
            y = w / (224.0 / 56)
            flag = 2
            out3 = self.bi_linear_sample(self.img_feat[2], n, x, y, flag)
            out3_list.append(out3)
        # ----
        all_out1 = tf.stack(out1_list, 0)
        all_out2 = tf.stack(out2_list, 0)
    ...
    def bi_linear_sample(self, img_feat, n, x, y, flag):
        x1 = tf.floor(x)
        x2 = tf.ceil(x)
        y1 = tf.floor(y)
        y2 = tf.ceil(y)
        if flag == 0:
            x2 = tf.minimum(223.0, x2)
            y2 = tf.minimum(y2, 223.0)
        if flag == 1:
            x2 = tf.minimum(111.0, x2)
            y2 = tf.minimum(y2, 111.0)
        else:
            x2 = tf.minimum(55.0, x2)
            y2 = tf.minimum(y2, 55.0)
        ...
        # zz= tf.stack([n, tf.cast(x2, tf.int32), tf.cast(y2, tf.int32)], 1)
        # print(zz)
